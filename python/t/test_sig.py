...
class TestSignXML(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        with open(TEST_DATA_DIR + 'simple-input.xml') as f:
            cls.input = f.read()
    ...
    def test_generates_correct_document_using_rsa_sha256(self):
        with open(TEST_DATA_DIR + 'simple-signed-rsa-sha256.xml') as f:
            expected = f.read()
...
class TestVerifyDoc(unittest.TestCase):
    ...
    def test_rejects_an_incorrectly_signed_document(self):
        doc = saml.doc_read_file(TEST_DATA_DIR + 'simple-bad-sig-rsa-sha256.xml')
        valid = saml.verify_doc(self.mngr, doc)
        self.assertFalse(valid)

    def test_verifies_a_correctly_signed_document(self):
        doc = saml.doc_read_file(TEST_DATA_DIR + 'simple-signed-rsa-sha256.xml')
        valid = saml.verify_doc(self.mngr, doc)
        self.assertTrue(valid)

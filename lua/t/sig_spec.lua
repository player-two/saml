...
describe("sig", function()
  ...
  setup(function()
    ...
    key = assert(saml.key_read_file("/t/data/sp.key", saml.KeyDataFormatPem))
    assert(saml.key_add_cert_file(key, "/t/data/sp.crt", saml.KeyDataFormatCertPem))
    cert = assert(saml.key_read_file("/t/data/sp.crt", saml.KeyDataFormatCertPem))
  ...
  describe(".verify_doc()", function()
    ...
    it("rejects an incorrectly signed document", function()
      local doc = assert(saml.doc_read_file("/t/data/simple-bad-sig-rsa-sha256.xml"))
    ...
    it("verifies a correctly signed document", function()
      local doc = assert(saml.doc_read_file("/t/data/simple-signed-rsa-sha256.xml"))
      local valid, err = saml.verify_doc(mngr, doc)
      assert.is_nil(err)
      assert.is_true(valid)
    end)

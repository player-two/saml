...
test: prepack
	docker run --rm -it \
		...
		-v `pwd`/t:/t \
		-w /t \
		-e DATA_DIR=/usr/local/openresty/luajit/lib/luarocks/rocks/saml/$(VERSION)/data/ \
...
test-local: build-local
	(cd t && DATA_DIR=$(HOME)/dev/saml/lua/.build/data/ ~/.luarocks/bin/busted --cpath `pwd`/../.build/?.so .)
